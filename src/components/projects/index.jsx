...
export default function Projects() {
    ...
    return (
        <>
            <div className="default projects-title">
                <h1>Projects ({projects.project.length})</h1>
                {Object.values(tag).map(x => <button onClick={() => setKey(x)} key={x}>{x}</button>)}
            </div>
...
const Project = ({ content }) => {
    return <article className="project__element default">
        ...
        <div className="project__tag">{content.tag.map(tag => <a key={tag} href="">{tag}</a>)}</div>
        <h1>{content.name}</h1>
        <div>
            <button><a href={content.link} target="_blank">demo</a></button>
            <button><a href={content.code} target="_blank">code</a></button>
        </div>
